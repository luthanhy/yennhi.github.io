...
function dist(...args) {
  ...
  if (args.length == 2) {
    if (typeof args[0] == "object" && typeof args[1] == "object") {
      xpart = args[1].x - args[0].x;
      ypart = args[1].y - args[0].y;
    } else if (typeof args[0] == "number" && typeof args[1] == "number") {
      xpart = args[0];
      ypart = args[1];
    } else {
      console.error("incorrect types of parameters");
    }
...
window.onload = function () {
  ...
  class node {
    ...
    follow(m) {
      ...
      this.td = Math.sqrt(Math.pow(this.errx, 2) +
                Math.pow(this.erry, 2));
      this.ta = Math.atan2(this.erry, this.errx);
      this.x = this.x + this.td * 0.2 
              * Math.cos(this.ta);
      this.y = this.y + this.td * 0.2 
              * Math.sin(this.ta);
    }
